...
writedlm("final_test_outside_epsilon_init.txt", ε_init)
...
savefig("final_test_outside_mode_init.pdf")
close("all")

imshow(real.(ε_init), cmap="gray_r", vmin=1, vmax=12)
colorbar(label=L"ε_{init}")
savefig("final_test_outside_epsilon_init.pdf")
close("all")

LDOS_opt, ε_opt, LDOS_vals, omegas, _, _ = smooth_LDOS_Optimize2d(Lx, Ly, ε_init, ω, b; resolution=res, ftol=0, max_eval=250000, design_dimensions=(3,3), fR=0.2, β=Inf)
Qs = -real.(omegas) ./ 2imag.(omegas)
writedlm("final_test_outside_epsilon_opt.txt", ε_opt)
writedlm("final_test_outside_LDOS_vals.txt", LDOS_vals)
writedlm("final_test_outside_Qs.txt", Qs)
...
savefig("final_test_outside_mode_opt.pdf")
close("all")

imshow(real.(ε_opt), cmap="gray_r", vmin=1, vmax=12)
colorbar(label=L"ε_{opt}")
savefig("final_test_outside_epsilon_opt.pdf")
close("all")

mod_LDOS_opt, mod_ε_opt, mod_LDOS_vals, mod_omegas, x, y = mod_smooth_Optimize2d(Lx, Ly, ε_init, ω, b, vec(b); resolution=res, ftol=0, max_eval=250000, design_dimensions=(3,3), fR=0.2, β=Inf, sgn=1)
mod_LDOS_vals =  vcat(fill.(mod_LDOS_vals, 2)...)
mod_Qs = -real.(mod_omegas) ./ 2imag.(mod_omegas)
mod_Qs = vcat(fill.(mod_Qs, 2)...)
writedlm("final_test_outside_mod_epsilon_opt.txt", mod_ε_opt)
writedlm("final_test_outside_mod_LDOS_vals.txt", mod_LDOS_vals)
writedlm("final_test_outside_mod_Qs.txt", mod_Qs)
...
savefig("final_test_outside_mod_mode.pdf")
close("all")

imshow(real.(mod_ε_opt), cmap="gray_r", vmin=1, vmax=12)
colorbar(label=L"mod ε_{opt}")
savefig("final_test_outside_mod_epsilon_opt.pdf")
close("all")
...
ylabel("LDOS")
savefig("final_test_outside_LDOS_iterations.pdf")
close("all")
...
savefig("final_test_outside_Q_iterations.pdf")
close("all")