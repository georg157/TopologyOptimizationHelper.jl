...
## ----- UNSHIFTED OPTIMIZATION (500,000 ITERATIONS) ----- ##

LDOS_opt, ε_opt, LDOS_vals, omegas, _, _ = smooth_LDOS_Optimize2d(Lx, Ly, ε_init, ω, b; resolution=res, ftol=0, max_eval=500_000, design_dimensions=(3,3), fR=0.2, β=Inf)
writedlm("log_epsilon_opt.txt", ε_opt)
writedlm("log_LDOS_vals.txt", LDOS_vals)
Qs = -real.(omegas) ./ 2imag.(omegas)
writedlm("log_Qs.txt", Qs)

## ----- SHIFTED OPTIMIZATION (500,000 ITERATIONS) ----- ##

mod_LDOS_opt, mod_ε_opt, mod_LDOS_vals, mod_omegas, x, y = mod_smooth_Optimize2d(Lx, Ly, ε_init, ω, b, vec(b); resolution=res, ftol=0, max_eval=500_000, design_dimensions=(3,3), fR=0.2, β=Inf, sgn=1)
