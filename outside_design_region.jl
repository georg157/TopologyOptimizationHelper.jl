...
_, ε_8, _, _, _, _ = smooth_LDOS_Optimize2d(Lx, Ly, ε, ω, b; resolution=res, ftol=0, max_eval=200, design_dimensions=(3,3), fR=0.2, β=8)

_, ε_16, _, _, _, _ = smooth_LDOS_Optimize2d(Lx, Ly, ε_8, ω, b; resolution=res, ftol=0, max_eval=500, design_dimensions=(3,3), fR=0.2, β=16)

_, ε_40, _, _, _, _ = smooth_LDOS_Optimize2d(Lx, Ly, ε_16, ω, b; resolution=res, ftol=0, max_eval=1000, design_dimensions=(3,3), fR=0.2, β=40)

_, ε_init, _, _, _, _ = smooth_LDOS_Optimize2d(Lx, Ly, ε_40, ω, b; resolution=res, ftol=0, max_eval=2000, design_dimensions=(3,3), fR=0.2, β=Inf)
writedlm("outside_design_region_epsilon_init.txt", ε_init)
...
savefig("outside_design_region_mode_init.pdf")
close("all")

imshow(real.(ε_init), cmap="gray_r", vmin=1, vmax=12)
colorbar(label=L"ε_{init}")
savefig("outside_design_region_epsilon_init.pdf")
close("all")
...
writedlm("outside_design_region_epsilon_opt.txt", ε_opt)
writedlm("outside_design_region_LDOS_vals.txt", LDOS_vals)
writedlm("outside_design_region_Qs.txt", Qs)
...
savefig("outside_design_region_mode_opt.pdf")
close("all")

imshow(real.(ε_opt), cmap="gray_r", vmin=1, vmax=12)
colorbar(label=L"ε_{opt}")
savefig("outside_design_region_epsilon_opt.pdf")
close("all")
...
writedlm("outside_design_region_mod_epsilon_opt.txt", mod_ε_opt)
writedlm("outside_design_region_mod_LDOS_vals.txt", mod_LDOS_vals)
writedlm("outside_design_region_mod_Qs.txt", mod_Qs)
...
savefig("outside_design_region_mod_mode.pdf")
close("all")

imshow(real.(mod_ε_opt), cmap="gray_r", vmin=1, vmax=12)
colorbar(label=L"mod ε_{opt}")
savefig("outside_design_region_mod_epsilon_opt.pdf")
close("all")
...
ylabel("LDOS")
savefig("outside_design_region_LDOS_iterations.pdf")
close("all")
...
savefig("outside_design_region_Q_iterations.pdf")
close("all")